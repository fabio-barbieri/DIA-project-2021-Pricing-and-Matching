...
def step3(m):
    ...
    for i in range(0, dim):
        for j in range(0, dim):  # are we assuming that the adjacency matrix is a square matrix?
            if m[i, j] == 0 and np.sum(assignments[:, j]) == 0 and np.sum(assignments[i, :]) == 0:
                assignments[i, j] = 1
                assigned = np.append(assigned, i)  # we utilize this array in order to store the rows containing an
                                                   # an assignment, and then we'll utilize this in order to initialize
                                                   # the array of the marked rows, that are the rows that do not
...
def step5(m, covered_rows, covered_columns):
    uncovered_rows = np.setdiff1d(np.linspace(0, m.shape[0] - 1, m.shape[0]), covered_rows).astype(int)
    uncovered_cols = np.setdiff1d(np.linspace(0, m.shape[1] - 1, m.shape[1]), covered_columns).astype(int)
...
# THE FUNCTION FOR COMPUTING THE FINAL ASSIGNMENT CAN BE DIVIDED IN TWO OR MORE FUNCTIONS
# now we implement the function that computes the assignments for lines with a single zero
...
# this is instead the function that computes the assignments for lines with more than one zeroes
def first_zero(m):
